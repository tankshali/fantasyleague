    ...
    for(var i = 0; i < total_number_of_configurations; i++) {
      var start_config = function(_configuration) {
        _configuration.start(function(err) {    
          ...
          var toGo = options.filters.length;

          // Execute all after configuration methods
          for(var k = 0; k < options.filters.length; k++) {
            options.filters[k].afterConfigurationStart(_configuration, function(err, result) {
              // Throw on error
              if(err) throw err;
              // Adjust number of methods to wait for
              toGo = toGo - 1;              

              // If done execute the actual function
              if(toGo == 0) {
                executeFunction();
              }
            });
